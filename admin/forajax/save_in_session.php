...
if (isset($_SESSION["cart"])) {
    ...
    if ($check_available == 0) {
        ...
        if ($available_qty > $qty) {
            ...
            echo "Enter qty is not available";
        }
    } else {
        $av_qty = 0;
        $exist_qty = 0;
...
function check_qty($company_name, $product_name, $product_unit, $packing_size, $link)
{
    $product_qty = 0;
    $res = mysqli_query($link, "SELECT * FROM stock_master WHERE product_company='$company_name' AND product_name='$product_name' AND product_unit='$product_unit' AND packing_size='$packing_size'");
...
function check_duplicate_product($company_name, $product_name, $product_unit, $packing_size)
{
    $found = 0;
    $max = sizeof($_SESSION['cart']);
    for ($i = 0; $i < $max; $i++) {
        if (isset($_SESSION['cart'][$i])) {
            ...
            if (
                $company_name_session == $company_name &&
                $product_name_session == $product_name &&
                $unit_session == $product_unit &&
                $packing_size_session == $packing_size
            ) {
                $found = $found + 1;
            }
        }
